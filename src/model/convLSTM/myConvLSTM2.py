...
class myConvLSTM:
    ...
    def parse_function(self,samples,label, info):
        ...
        lstm_input = tf.reshape(samples, shape=[samples.shape[0],samples.shape[1]]) 
       # label = tf.reshape(label, shape=[label.shape[0]]) 
        return lstm_input , label, info, samples
...
#            self.y = tf.placeholder(tf.int32, shape=[None,cfg.NN_Dense['n_outputs']], name="y")  
            ...
            self.weights = {
                        ...
                        'wd1': tf.Variable(tf.random_normal([(cfg.NN_LSTM['num_unitsLSTM']+32*8), cfg.NN_Dense['n_fc1']])),                                             
        ...
        with tf.name_scope("Conv2D"):  
            # Convolution Layer
            conv1 =  self.myLayers.conv2d(self.next_element[3], self.weights['wc1'], self.biases['bc1'],1,1,"conv1")             
...
#            self.conv6flat = tf.reshape(self.conv6, shape=[-1, 1,math.ceil(cfg.NN_input['height']/6)*math.ceil(cfg.NN_input['width']/8)*cfg.NN_CNN['conv6_fmaps']]) #/8 weil 3 mal maxpool mit k=2, also : /2/2/2
            self.conv6flat = tf.reshape(self.conv6, shape=[-1, self.conv6.shape[1]* self.conv6.shape[2]*self.conv6.shape[3]])
            self.LSTMinput = tf.reshape(self.next_element[0], shape=[-1, self.next_element[3].shape[1], self.next_element[3].shape[2]])
        with tf.name_scope("LSTM"):    
             top_layer_h_state =  self.myLayers.LSTMLayer(self.LSTMinput)
             
        with tf.name_scope("fc"):
            
            denseinput = tf.concat([self.conv6flat, top_layer_h_state], 1) 
