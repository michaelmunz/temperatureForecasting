...
class myConvLSTM:
    ...
    ############### GET LAYERS ##########################
        ...
        self.seq_len = tf.placeholder("int64", [1], name="seq_len_placeholder")
...
#            self.y = tf.placeholder(tf.int32, shape=[None,cfg.NN_Dense['n_outputs']], name="y")  
            ...
            self.weights = {
                        ...
                        'wd1': tf.Variable(tf.random_normal([cfg.NN_LSTM['num_unitsLSTM']+(cfg.NN_input['width']*cfg.NN_input['height']), cfg.NN_Dense['n_fc1']])),                                             
...
#            self.conv6flat = tf.reshape(self.conv6, shape=[-1, 1,math.ceil(cfg.NN_input['height']/6)*math.ceil(cfg.NN_input['width']/8)*cfg.NN_CNN['conv6_fmaps']]) #/8 weil 3 mal maxpool mit k=2, also : /2/2/2
            self.conv6flat = tf.reshape(self.conv6, shape=[-1, 1, cfg.Modelparameter_highlevel['batchsize']*self.conv6.shape[1]* self.conv6.shape[2]*self.conv6.shape[3]])
#            self.conv6flat = tf.reshape(self.conv6, shape=[-1, self.conv6.shape[0]/5, self.conv6.shape[1]* self.conv6.shape[2]*self.conv6.shape[3]])
        with tf.name_scope("LSTM"):    
            top_layer_h_state =  self.myLayers.LSTMLayer(self.conv6flat)
         
        #with tf.name_scope("LSTM2"):  
            flatted_input = tf.reshape(self.next_element[0], shape=[1, self.next_element[0].shape[0]*self.next_element[0].shape[1]*self.next_element[0].shape[2]])
#            top_layer_h_state2 =  self.myLayers.LSTMLayer(flatted_input, name = "LSTM-Cells2")
        ...
        with tf.name_scope("fc"):
            denseinput = tf.concat([top_layer_h_state, flatted_input], 1)
            fc1 =  self.myLayers.fcLayer(denseinput,self.weights['wd1'],self.biases['bd1'], self.training)
